...
function tamañodePaso(t0 :: Number, tf :: Number, N :: Number)
	h = (tf-t0)/N
end

# ╔═╡ d5de8715-aa38-48c9-8da3-7c3f6787a344
begin
	tf=9 #Definimos las variables de TamañodePaso
	t0=0
	N=5
end

# ╔═╡ f92919fe-9ce7-4b46-8110-57ba7c4b5116
tamañodePaso(t0,tf,N)
...
# ╔═╡ 5e8b9bd1-4749-4345-8aee-0e42f443c82a
h=tamañodePaso(t0,tf,N) 

# ╔═╡ b0eb99e9-978a-4914-90a9-e05c73115e73
function arreglouniforme(t0 :: Number, tf :: Number, N :: Number, h :: Number)

	if N<0
		println("Error, N debe ser mayor a cero")
	elseif 0<N
	for i in 0:N
		ti= t0 + i*h
		println(ti)
...
# ╔═╡ ed8e2c71-f4f3-412d-8d8a-a2f77c0ae1e0
arreglouniforme(t0,tf,N,h)
...
# ╔═╡ 3c574b0c-0b69-4439-8d9b-f819c3e97742
begin
ti=0
xti=exp(ti)
	g=exp
...
function paso_euler(ti::Number, xti:: Number, g::Function , h :: Number) #Definimos nuestra funcion Pasoeuler

	xti=exp(ti)  #comenzamos evaluando ti en la funcion x
	x(xti)=xti + h*g(xti) #Definimos la ecuacion que se estara iterando
	
for i in collect(1:N)   #Para cada i en N
	
	xti  = x(xti)   #Actualizamos el valor de cada xti 
println(xti)  #Imprime xti1
	end
	end


# ╔═╡ c0fcaa1e-aac4-47c1-b2d0-dcfc61d6930a
paso_euler(ti,xti,g,h)
...
function euler(g′3::Function, xt08 :: Number) #Definimos nuestra funcion euler

	 v1=hcat(xt08,0) #Imprime nuestra condicion inicial
	
	println(v1) #Imprime el vector 5,0, que es el valor [x0,t0]
	
	for i in 0:N #Para cada elemento de 0:N

    t(xt08)= xt08 + i*h # ti+1 = ti + ih
	xt08=t(xt08)	#y actualizamos el valor para iterar los resoltados

		c=xt08[ xt08 .>0]	
	x(xt08)  = xt08 + h*g′3(xt08) #a cada valor de nuestras ti+1 lo evaluamos en la siguiente funcion 

       v2=x(xt08)[x(xt08) .> 0]
		

		l= hcat(v2,c) #Une los conjuntos de datos

			println(l)   #Imprime el vector de los valores [x(t_{i+1}), ti]
	
end	
end

# ╔═╡ bfe52cf5-38d9-4862-996a-b4d6c829c056
@bind  M Slider(0:2:10)

# ╔═╡ c992ae3c-cb42-4cfe-9df3-1d9aed0d428a
begin 
g′3= exp
	xt08=5
end

# ╔═╡ 2ff85881-35cd-4eee-955a-09591eb960ce
euler(g′3, xt08) #Ejecutamos nuestra funcion
...
Si nuestra función $g$ únicamente depende del tiempo, tendriamos la siguiente relación:

$$\begin{align*} x'(t) &= g(t), & \\ x(t_0) &= x_0 & \end{align*}$$

Podemos observar que se trata de unicamente una derivada, y para conocer el valor de $x′(t)$ bastaria con integrar para conocer el valor de $x(t)$.
...
# ╔═╡ 7ec54425-b678-4d64-a2a4-616a4be4e2c3

function euler2(g′4::Function, xt06 :: Number) #Definimos nuestra funcion euler

	 v1=hcat(xt06,pi/2) #Imprime nuestra condicion inicial
	
	println(v1) #Imprime el vector 5,0, que es el valor [x0,t0]
	
	for i in 0:N #Para cada elemento de 0:N

    t(xt06)= xt06 + i*h # ti+1 = ti + ih
	xt06=t(xt06)	#y actualizamos el valor para iterar los resoltados

		c=xt06[ xt06 .>0]	
	x(xt06)  = xt06 + h*g′4(xt06) #a cada valor de nuestras ti+1 lo evaluamos en la siguiente funcion 
...
# ╔═╡ 64b2943a-5e53-49b0-b0c9-cccf86a7e926
euler2(g′4,xt06) #Observamos que nuestra condicion inicial x0=1, en nuestro codigo imponemos esta condicion y t0 es pi/2
#ademas observamos que la columna 1 son las xi+1 correspondientes y la columna 2 con las ti correspondientes, entonces si evaluamos
# x(t_{i+1})= x(ti) + h g(ti), es decir el vector obtenido es el conjunto [x(t_{i+1}),ti]

# ╔═╡ 8198d45f-9ee4-4bbc-bce3-398eca8dde3a
f2=[0,5,6.8,10.4,15.8,23.0,32.0]

# ╔═╡ 8c5445d0-cf81-4dac-baeb-4043102c813f
f1= [5,272.1436863846379,1622.9251249707515,59157.726213997994,1.309561332510127e7,1.7540646226248028e10,1.4213332832885725e14]

# ╔═╡ 5d1d1a28-67c1-4a07-b44f-741d01dababa
begin  #Se puede observar que en la grafica se sigue una tendendcia exponencial, sin embargo por las dimensiones de los ejes.
	# No se aprecia con claridad la condicion inicial, sin embargo lo pintamos de rojo para observar que se cumple.
scatter((f2,f1), xlims=(-10,40), ylims=(-10000000000,3.5e10), color = "blue" , title=  "Método de Euler" , label= false,xlabel=L"t_i", ylabel=L"x(t_{i+1})"  )
	scatter!((0,5), color="red", label="Condicion Inicial")
