...
begin
	plot(sin, 0:0.25:2π, title = "plot con atributo 'marker = true'", xlabel = "x", ylabel =  L"\sin(x)", color = "darkorange", label = "plot", marker = true)
end
...
# ╔═╡ baad9904-8d18-4fc6-855a-8ce147b0a362
@bind  d Slider(0.05:0.1:2, default=0.1)
#Asignamos un valor a 'd' a través de un deslizador interactivo que toma valores en 0.05:0.05:2.

# ╔═╡ 3d3c7901-a0bd-48e7-ae53-05ca8660fe81
d

# ╔═╡ 3df51026-6bc2-4169-a777-47ef9070fe71
begin
	plot(sin, -π:d:π, title = "Funciones trigonométricas" ,  ylabel =  L"\sin(x)", color = "red", label =  L"\cos(x)", marker = true)
	plot!(cos, -π:d:π, xlabel = L"x ", ylabel =   L"f(x)"  , color = "blue", label = L"\sin(x)", marker = true)
end
...
# ╔═╡ 698315ec-0265-4e2a-9649-aa0f959e7b6b
begin

H, r, θ = 30, 5, π/6 #Comenzamos definiendo las variables H, r y θ

	
v_x=r*cos(θ)    #Indican las componentes del vector velocidad de la particula
v_y=r*sin(θ)

w(x)= ((v_x)*x, H +(v_y)*x -9.81*x^2/2)
w_x(x) = (v_x)*x
w_y(x) = H +(v_y)*x - 9.81*x^2/2

x_max= v_x*((2*v_y/9.81+((2*v_y/9.81)^2 + 8*H/9.81)^(1/2))/2)  #Indica las posiciones máximas en las respectivas coordenadas x Y y
y_max= H + (v_y)*(v_y/9.81) - 9.81*(v_y/9.81)^2/2
		              

	anima = @animate for t in 0:0.1:7 # Por cada t ∈ 0:0.1:7,
	plot(w.( range(-(2),0, step = 0.1) .+t),      #grafica la funcion definida w
		 legend = false, title = "Tiro Parabólico", lw=8, ls=:dot, xlabel = L"x ", ylabel =   L"H"  , color = "red", xlims=(0,x_max), ylims=(0,y_max)) # Traza la línea punteada
...
# ╔═╡ c3264b4d-81b1-4e0c-9205-ff818665788c
 begin
anim4 = @animate for t in 0:0.1:7 #Para cada t en 0:0.1:7
	plot(w.( range(-(2),0, step = 0.1) .+t),      #grafica la funcion definida w en la celda anterior descrita.
		 legend = false, title = "Tiro Parabólico", lw=8, ls=:dot,xlabel = L"x ", ylabel =   L"H" , xlims=(0,x_max), ylims=(0,y_max) ) 
		scatter!([w_x(t)],[w_y(t)])
	end 
	 gif(anim4, "Tiro_Parabólico.gif", fps=30)
...
g(x)= cos(x) + sin(x) #Consideramos la aproximacion cuando x tiende a y
	
anim7  = @animate for q in 0:0.1:2π 	#para cada q en 0:0.1:2pi
plot(g.( range(0,2pi, step = 0.1).-q ),      #grafica la funcion definida g
		 legend = false, title = "Superficie de funcion h",xlabel = L"t ", ylabel =  L"h(x)" ,xlims=(0,60), ylims=(-2,1.5) ) 

end 

	 gif(anim7, "SuperficiedefuncionH.gif", fps=30)
end
...
# ╔═╡ 9432b768-e698-4d36-9aee-bf0f8dd90ade
