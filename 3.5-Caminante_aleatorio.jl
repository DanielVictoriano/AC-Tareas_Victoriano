...
# ╔═╡ af184d80-4f31-494f-83f8-3d22e847371b
using Plots, StatsPlots

# ╔═╡ 2fbfe1af-9282-435e-93cf-0bbb39ba406f
using Random123

# ╔═╡ 6e0b19c1-0c23-4c53-bcbc-7170861ca76c
using FoldsCUDA

# ╔═╡ d1447396-5071-4078-b5ad-c9439a5c4e73
using PlutoUI

# ╔═╡ 79acbc79-025f-4dba-b279-4859d0285725
using CUDA
...
# ╔═╡ f47746f7-5b6a-4a97-b563-8bc62e4bceeb
...
# ╔═╡ 3dc81542-242e-42e2-871c-6e69a73732ab
A = collect(1:50)

# ╔═╡ 0878eece-266f-47ce-9385-3ca01e0e4dfd
r= rand(Bool,6)

# ╔═╡ 0f825c5e-cbd0-448e-bbfc-5d84c99e0845
rand(1:5)
...
begin  #Definimos las variables con las que vamos a trabajar
a=0
b=10

function aleatorioUniforme(a::Number,b::Number) #Nonbramos nuestra funcion 
	x = a + rand() *(b-a) #x nos brinda un numero aleatorio de a:b
    return x
	println(x)
end
end
...
# ╔═╡ a218c96a-1d34-4fc3-b3a7-5fdeb7586e20
aleatorioUniforme(a,b) #Ejecutamos nuestra funcion con argumentos a,b 

# ╔═╡ 41a80720-ab01-4931-b86f-1c72cf48c993
begin
	u= [aleatorioUniforme(a,b) for _ in 1:100000] #Definimos un vector con la funcion definida anteriormente
histogram(u, bins=10, title="Histograma" ) #Generamos el histograma 
end
...
function  unPaso() #Definimos la funcion unPaso
	if rand(Bool)== true  #Si rand(Bool) es true imprime 1
		println(1)
	else rand(Bool)== false #Si rand(Bool) es false imprime -1
		println(-1) 
	end
end

# ╔═╡ 76bc3061-b904-4473-8d3e-eab5b87f464b
unPaso()
...
function  variosPasos(n::Number)
d = rand(Bool,n) #Generamos nuestro valor 0 o 1, y siempre se multiplicara por 2 y se le restara 1
	d= 2 .* d .-1
end


# ╔═╡ 3bc9a8b8-ad53-4da4-80cc-ae349a8bde9b
d4=variosPasos(32)

# ╔═╡ c83d41ed-3d65-4949-8e10-a25ed273d13e
d3=variosPasos(18)

# ╔═╡ c42bdd65-7d4e-4503-83fa-b166d0f67025
d2=variosPasos(29)

# ╔═╡ 76d3c712-a6ca-4ab6-b94b-994c71f629c2
d1=variosPasos(25)
...
function graficaCaminata(ca) #Definimos la funcion
	k = length(ca)   #Obtenemos la cantidad de elementos que tiene el vector ca
	pa2 = 0:k-1 #definimos los pasos como el vector desde 0 a k-1 pasos
	scatter((pa2,ca), title="Caminata Aleatoria", xlabel="Numero de Pasos", ylabel="Posicion") #Obtenemos la grafica
end
...
# ╔═╡ 49301169-730b-4b19-9706-db2e8fa6e67f


# ╔═╡ 64ce411e-e61c-488f-a304-2df84b64ac79


# ╔═╡ 370dff98-2678-474d-9f45-113d2305e40d
n1 = rand(A)

# ╔═╡ accf3d4d-0f9d-46be-a201-d6c659ee8d43
 d=variosPasos(n1) #Ejecutamos la funcion

# ╔═╡ 9d09fc0f-ccba-477d-9968-82b9554f44e2
function caminataAleatoria(n::Number,d::Vector{Int64}) #Definimos la funcion 
	pa = variosPasos(n1) #Denominaos al numero de pasos 
	ps = cumsum(d) #Definimos la posicion ps como la suma acumulativa del vector d

	ca= [0;ps]#Definimos el vector con la posicion 0 y ultimo valor ps
	
	return ca

end

# ╔═╡ f44d7a03-503f-40a0-a3c3-f7faaa40cf75
caa3 =  caminataAleatoria(32,d4)

# ╔═╡ 3165b3f0-27dc-4661-9830-b5689699d5f3
caa2 =  caminataAleatoria(18,d3)

# ╔═╡ 72ed5cf4-1e99-45cb-a26f-dbe100ff3ed7
caa1 =  caminataAleatoria(29,d2)

# ╔═╡ 1e76d10c-b72b-4ee6-b904-db2db4b9e013
caa =  caminataAleatoria(25,d1)

# ╔═╡ d82b459a-1c3d-43bb-b5e3-51935db23d38
ca = caminataAleatoria(n1,d) #Ejecutamos la funcion

# ╔═╡ 5ba2ec6d-8ec9-4db3-a7f6-e051f6946ae9
graficaCaminata(ca)

# ╔═╡ 896d6213-105b-403f-8aed-632303c7ba2b
begin

function graficaCaminata!(cams1) #Definimos la funcion
	pa2= 0:length(ca)-1
	
 plot!((pa2,ca) , linestyle= :dash, marker= :circle,label=false, title="Caminata Aleatoria", xlabel="Numero de Pasos", ylabel="Posicion", ) 	#Obtenemos la grafica
end
end



# ╔═╡ 66b6d3f1-17fe-4376-a3d2-ad78a56efa2f
begin
cams1= [caminataAleatoria(40)  	 ]
		
graficaCaminata!(cams1)


end
	

# ╔═╡ effe1087-8458-44fe-a53c-5d4f974e2742
begin 
	pp = 0:length(ca)-1
	pp1= 0:length(caa)-1
	pp2 = 0:length(caa1)-1
	pp3= 0:length(caa2)-1
	pp4= 0:length(caa3)-1
plot!((pp,ca),linestyle=:dash, marker=:circle, xlims=(0,50),ylims=(-4,10),xlabel="Posición Horizontal", ylabel="Posición Vertical", label=false, title="Caminante Aleatorio")
plot!((pp1,caa),linestyle=:dash, marker=:circle, label=false)
plot!((pp2,caa1),linestyle=:dash, marker=:circle, label=false)
plot!((pp3,caa2),linestyle=:dash, marker=:circle, label=false)
plot!((pp4,caa3),linestyle=:dash, marker=:circle, label=false)

end

# ╔═╡ 2b56edaa-76a3-46f9-af36-912a0b7d2215
graficaCaminata!(ca)
...
# ╔═╡ e85464a5-1ce7-4095-9372-1c7e6a25cad6
begin #Definimos nuestra funcion

	function animaCaminata2(ca)     
		k=length(ca)  #El numero de elementos de nuestro vector
		pa= 0:k-1 #los pasos 
		an = @animate for i in 1:k  #para cada elemento del vector 1:k
			plot(ca[1:i],linestyle=:dash, marker=:circle, xlims=(0,60),ylims=(-4,5),xlabel="Posición Horizontal", ylabel="Posición Vertical", label=false, title="Caminante Aleatorio")  #anima grafica caminata desde el primer elemento de ca hasta el elemento i=k
		end
		return an
	end
...
# ╔═╡ 3401a428-d2c5-4cea-b9fb-4a95aaa64853
begin
	an = animaCaminata2(ca)  
	gif(an, fps=1)
end
...
function caminataAleatoria2D(n) #Definimos nuestra funcion
 phn = [0.0]  #Iniciales con vectores posicion horizontal y vertical en cero
 pvn = [0.0]
	for i in 1:n  #Para cada 1 en 1:n
		pah = aleatorioUniforme(-1.0,1.0)  #Definimos al paso horizontal y vertical como el valor dado por la funcion aleatorioUniforme
		pav = aleatorioUniforme(-1.0,1.0)

		ph = phn[end] + pah  #Definimos a la posicon horizontal como las posiciones iniciales + el paso aleatorio uniforme
		pv = pvn[end] + pav

		push!(phn, ph)  #Unimos los datos obtenidos
		push!(pvn, pv)
		
end
return[phn,pvn]	#Obtenemos el conjunto de ambos vectores
end
...
function graficaCaminata2D!(cams)
for ca3 in cams
	phn = ca3[1]
	pvn = ca3[2]	
	plot!( phn, pvn, linestyle= :dash, marker= :circle, label=false)
end
	 xlabel!("Posición Horizontal")
    ylabel!("Posición Vertical")
end

# ╔═╡ 3e5bd352-0323-4286-b1ae-b9ca5b46b8b2
begin
@bind n3 Slider(1:3:150, default=20)
end

# ╔═╡ f38cef58-ba61-4410-8d4e-4230dd53c9c5
ca2=caminataAleatoria2D(n3) #Ejecutamos la funcion para obtener el arreglo de los dos vectores

# ╔═╡ 6c61a77d-448f-4e63-8d70-b662608d7ec4
plot((ca2[1], ca2[2]), linestyle = :dash, marker = :circle, label = "Caminata Aleatoria", xlabel = "Posicion Horizontal", ylabel = "Posicion Vertical", legend = :topleft)

# ╔═╡ d292165a-b37b-47a6-a61e-dbdafbf1f928
begin 
ph1=ca2[1]
	pv1=ca2[2]
	function graficaCaminata2D(ca2)
	plot((ph1,pv1), linestyle = :dash, marker = :circle, label = "Caminata Aleatoria", xlabel = "Posicion Horizontal", ylabel = "Posicion Vertical", legend = :topleft) #Graficamos en base a los arreglos obtenidos
end
end

# ╔═╡ 90713b33-5308-4e84-9316-1b7457ac0dcc
graficaCaminata2D(ca2)

# ╔═╡ 05e7db24-7e5b-414f-9234-0661893554cf
begin 
	cams= [caminataAleatoria2D(n3) for _ in 1:5 ]
	plot()
	graficaCaminata2D!(cams)
end
...
function animaCaminata2D(ca3)
j = length(ca3[1])
	ani= @animate for i in 1:j
		plot(ca3[1][1:i], ca3[2][1:i], linestyle=:dash, marker=:circle, markersize=3, xlims=(-10, 10), ylims=(-10, 10), legend=:none, xlabel="Posición Horizontal", ylabel="Posición Vertical")
end
	gif(ani, "Cami.gif", fps=15)
end

# ╔═╡ b942dd1a-fd55-490e-a829-d865a1b0bc32
begin 
ca3= caminataAleatoria2D(n3)
animaCaminata2D(ca3)
end
...
pz=[0.0]
	for i in 1:100
		pax=  aleatorioUniforme(-1.0, 1.0)
		pay= aleatorioUniforme(-1.0, 1.0)
		paz= aleatorioUniforme(-1.0, 1.0)
...
# ╔═╡ ed712045-2f79-491b-9ca6-78821175f9b4
function graficaCaminata3D(px,py,pz)
plot3d(px, py, pz, linestyle=:solid, marker=:circle, xlabel="Eje x", ylabel="Eje y", zlabel="Eje z") #Utilizamos función de Plots para gráfica
end

# ╔═╡ 2e0f9bf1-693b-4cc8-bdf2-822f03968055
cax,cay,caz = caminataAleatoria3D(100)


# ╔═╡ 72a1fad5-4f4f-4abe-8147-f325a1637733
graficaCaminata3D(cax, cay, caz)
...
julia_version = "1.8.5"
manifest_format = "2.0"
project_hash = "b78ddd71a629bf852bd79b56c7e03a28d844a4a6"
