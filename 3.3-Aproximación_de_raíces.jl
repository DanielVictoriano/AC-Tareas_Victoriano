...
# ╔═╡ fd7c633c-a305-4751-b155-0a5a8459cae6
using Plots, PlutoUI

# ╔═╡ d509c74c-dc43-4cd6-9ae5-881f56ac4295
using Random123

# ╔═╡ 275f6fae-7dae-405f-88fc-370c584cc5fa
using LaTeXStrings

# ╔═╡ 9103ac95-47fc-4000-aa73-48cc5a311725
using FoldsCUDA

# ╔═╡ 4a09a589-0cd0-4d50-a24f-c6f5dc2a28ce
using FLoops

# ╔═╡ 11eaef38-8564-4b4a-b8c0-c9c1c2496232
using CUDA
...
# ╔═╡ 259f64a8-99a7-42a0-aaf5-3ff2d77aac6d
...
function Newton(f:: Function, x0 :: Number, f′ :: Function, n :: Number) #Nombramos a nuestra función Newton
    g(x0) = x0 - (f(x0)/f′(x0)) #Definimo el valor g(x0) inicial
for i in collect(1:n)  #Para cada i en 1 a n el valor anterior x0 sera g(x0)
	x0=g(x0)
	println(x0) #Imprime las x0 correspondientes
end
end

# ╔═╡ 14053de6-67c9-4a0e-8f3b-9c3e172dd7ad
begin
	f=exp
	f′=exp  #En esta celda definimos las variables a evaluar en nuetra funcion Newton
	x0=5
	n=5
end
...
# ╔═╡ 78ece7ef-6e30-48db-8026-5dfcd08eb041
Newton(f,x0,f′,n)
...
function NewtonRecursiva(f1:: Function, x01 :: Number, f′1 :: Function, n1 :: Number)
	 g1(x01) = x01 - (f1(x01)/f′1(x01)) #Definimo el valor g(x0) inicial
	...
	elseif n1 < 0
		...
		for i in collect(1:n1)  #Para cada i en 1 a n el valor anterior x0 sera g(x0)
	x01=g1(x01)
	println(x01) #Imprime las x0 correspondientes
...
# ╔═╡ 29b7d291-97d6-465a-a4ca-98a2f01a51d4
begin
	f1=sin
	f′1=cos #En esta celda definimos las variables a evaluar en nuetra funcion NewtonRecursiva
	x01=5
	n1=7
end


# ╔═╡ 7706b89e-7619-482b-b32e-0daad0df5589
NewtonRecursiva(f1,x01,f′,n1)
...
function NewtonDistancia(f2:: Function, x02 :: Number, f′2 :: Function,  ϵ :: Number)
	 g2(x02) = x02 - (f2(x02)/f′2(x02)) #Definimo el valor g(x0) inicial

	
	if ϵ <= 0    #Si ϵ es menor a cero imprime un mensaje de error
			println("Error!")
	else ϵ > 0   #Si ϵ es mayor a cero 
	while abs(g2(x02)-x02) <ϵ #Para cada i en 1 a n el valor anterior x0 sera g(x0)
	x02=g2(x02) 
	println(x02) #Imprime las x02 correspondientes
...
# ╔═╡ 64353035-9a5e-46de-8c6a-63c42ca9934d

begin
	f2=exp
	f′2=exp #En esta celda definimos las variables a evaluar en nuetra funcion NewtonRecursiva
	x02=5
	ϵ = 1.1
end

# ╔═╡ d3cf8c14-ffcf-49ee-95f5-11f3f815389d
NewtonDistancia(f2,x02,f′2,ϵ)
...
function ADE(f3 :: Function, x3 :: Number, h :: Number) #Generamos la funcion ADE, que es la aproximacion de la derivada
f′3= h*(f3((x3*h+1)/h)-f3(x3))
	println(f′3)
end

# ╔═╡ cd640c4b-578f-4ab7-930a-b5157a629cbc
begin
f3= sin  #Definimos nuestras variables de nuestra funcion ADE
x3=0
h= 1000000000000000
end

# ╔═╡ 68455c4c-68ef-476d-93db-2baa594948c2
ADE(f3,x3,h)
...
function NewtonDN(f4:: Function, x04 :: Number, n4:: Number, h :: Number)

f′4= h*(f((x04*h+1)/h)-f(x04))
	
	 g4(x04) = x04 - (f4(x04)/f′4) #Definimo el valor g(x04) inicial
	...
	elseif n4 < 0
		...
		for i in collect(1:n4)  #Para cada i en 1 a n el valor anterior x04 sera g(x04)
	x04=g4(x04)
	println(x04) #Imprime las x04 correspondientes
	
end
end
end

# ╔═╡ c0fb1b13-2cc3-44c0-a87b-78454deda96c
begin 
f4= exp
x04= 5
n4= 1
...
# ╔═╡ 372ad3ad-cc0c-4008-90ef-2f69bfc0c986
NewtonDN(f4,x04,n4,h1)
...
f5 = cos # Luego, usa esta celda para reasignar `f`

# ╔═╡ 5e1daa81-a9f1-43de-8e18-c62ca0dc1cae
@bind h5 Slider(1000:1000:1000000, default=1000) 

# ╔═╡ 20434aac-a75a-4214-9da0-4b4287285998
@bind n5 Slider(1:1000, default=1000) 
...
# ╔═╡ 42d538d4-b5a8-41f8-b57d-b195c9c6a4d3
@bind x05 Slider(-10:0.1:10, default=0) #Manipulamos la cantidad n de puntos con un Slider para una mejor visualización

# ╔═╡ fe80e8e9-6b83-4d22-8e23-8a3961f8f54d
xi = collect(-10:0.1:x05)

# ╔═╡ ca3ff80c-6915-48a3-9b09-45c2647253c5
for i in xi
	fii= f5(i)
end

# ╔═╡ 2b2fac55-2249-41a1-9b6b-ddeb600a3e89
fii=f5.(xi)

# ╔═╡ e9ccc12b-5078-4e0c-9f26-33bff35c74f7
begin
plot((xi,fii), label=false ) #Pone la función constante π
scatter!((xi,fii),  xlims=(-10.2,10.2), ylims=(-1.1,1.1), label=false, xlabel = L"x_i", ylabel =  L"f(x_i)") #Pone los valores aproximados de π en relación con la cantidad de puntos, así como la incertidumbre asociada al valor predeterminado.
end
...
julia_version = "1.8.5"
manifest_format = "2.0"
project_hash = "0d1e5d9ba65b5d2ba930953b7d7fc12b5bd7dbfb"
